...
CREATE TABLE IF NOT EXISTS pets (
  ...
  CONSTRAINT uq_pets_microchip_id UNIQUE (microchip_id)
);
...
CREATE TABLE IF NOT EXISTS pet_transfers (
  ...
  FOREIGN KEY (pet_id) REFERENCES pets(id),
  FOREIGN KEY (from_owner_id) REFERENCES owners(id),
  FOREIGN KEY (to_owner_id) REFERENCES owners(id),
  CONSTRAINT pk_pet_transfers PRIMARY KEY (id)
);

CREATE INDEX IF NOT EXISTS idx_pet_transfers_pet_id ON pet_transfers (pet_id);

ALTER SEQUENCE pet_transfers_id_seq RESTART WITH 100;
...
CREATE TABLE IF NOT EXISTS pet_photos (
  ...
  CONSTRAINT uq_pet_photos_pet_id UNIQUE (pet_id),

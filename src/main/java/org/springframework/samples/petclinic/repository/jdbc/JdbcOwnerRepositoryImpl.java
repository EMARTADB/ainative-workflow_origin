...
public class JdbcOwnerRepositoryImpl implements OwnerRepository {
    ...
    public void loadPetsAndVisits(final Owner owner) {
        final List<JdbcPet> pets = this.jdbcClient.sql("""
            SELECT pets.id, name, birth_date, type_id, owner_id, microchip_id, visits.id as visit_id, visit_date, description, pet_id, visits.vet_id
    ...
    public Owner findOwnerByPetMicrochipId(String microchipId) {
        List<Owner> owners = this.jdbcClient.sql("""
                SELECT owners.id, first_name, last_name, address, city, telephone
                FROM owners
                JOIN pets ON owners.id = pets.owner_id
                WHERE pets.microchip_id = :microchipId
                """)

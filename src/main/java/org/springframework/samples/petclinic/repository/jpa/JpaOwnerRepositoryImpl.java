...
public class JpaOwnerRepositoryImpl implements OwnerRepository {
    ...
    public Owner findOwnerByPetMicrochipId(String microchipId) {
        Query query = this.em.createQuery(
            "SELECT o FROM Owner o JOIN o.pets p WHERE p.microchipId = :microchipId");
        query.setParameter("microchipId", microchipId);
        java.util.List<?> results = query.getResultList();
        return results.isEmpty() ? null : (Owner) results.get(0);
    }
